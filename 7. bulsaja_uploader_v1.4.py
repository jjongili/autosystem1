...
class BulsajaUploader:
    def __init__(self, gui):
        ...
        # 통계
        self.stats = {"total": 0, "success": 0, "failed": 0, "skipped": 0, "failed_ids": [], "failed_product_ids": []}
    ...
    def log_ctx(self, product_id: str, msg: str):
        """멀티세션용 컨텍스트 로그 - 상품ID 접두사 포함"""
        # 상품 ID 마지막 6자리를 접두사로 사용
        short_id = product_id[-6:] if len(product_id) > 6 else product_id
        prefixed_msg = f"[{short_id}] {msg}"
        self.log(prefixed_msg)
    ...
    def process_product(self, product: Dict, group_name: str, option_count: int,
        ...
        try:
            ...
            # [v1.4+] 옵션 1개일 때 키워드 필터링 건너뛰기
            skip_keyword_filter = len(upload_skus) == 1
            if skip_keyword_filter:
                self.log(f"   ℹ️ 옵션 1개 → 키워드 필터링 건너뜀")
            ...
            # [v1.4+] 강제 제외 키워드: 공통키워드 통과 로직 무시 (항상 필터링)
            FORCE_EXCLUDE_KEYWORDS = {"품절", "재고없음", "단종", "판매종료", "환불", "sold out", "out of stock"}

            excluded_common_keywords = set()
            for kw, matching_skus in keyword_skus.items():
                # 강제 제외 키워드는 공통키워드 통과 안 됨
                if kw in FORCE_EXCLUDE_KEYWORDS:
                    continue
            ...
            if not valid_skus:
                self.log(f"   ⏭️ PASS: 클러스터 필터링 후 유효 옵션 없음")
                result['status'] = 'skipped'
            ...
            if 'uploadSkuProps' in detail:
                 ...
                 if max_text_dims >= 4:
                     ...
                     self.log(f"   ⏭️ PASS: {result['message']}")
                     return result
                ...
                for p_idx, product in enumerate(products, 1):
                    ...
                    for m_idx, current_market in enumerate(target_markets):
                        ...
                        if result['status'] == 'success':
                            ...
                            # 태그용 순수 ID 저장 + 즉시 태그 적용 (비동기)
                            raw_id = result.get('id')
                            if raw_id and raw_id != '?' and raw_id not in self.stats['failed_product_ids']:
                                self.stats['failed_product_ids'].append(raw_id)
                                self._tag_failed_async(raw_id)  # 별도 스레드로 즉시 태그 적용
                        self.stats['total'] += 1
