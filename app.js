        ...
        return `
            <div class="monitor-card status-${status}" 
                ...
                <div class="expand-info">
                    ...
                    <div class="expand-row cleanup-row ${acc.cleanup_status || 'normal'}">📅 마지막등록: ${acc.last_cleanup_date || '-'} (${acc.days_since_cleanup || 0}일전)</div>
                </div>
            </div>
        `;
    }).join('');
    ...
    document.querySelectorAll('.tab').forEach(tab => {
        tab.addEventListener('click', () => {
            if (tab.dataset.tab === 'monitor') {
                loadMonitorData();  // 관제센터 데이터 로드 (product-counts 포함)
                // loadDailyStatus();  // 주석처리: dailyGrid를 덮어쓰는 문제 해결
...
let cleanupTiers = [20, 30, 45, 60];  // 경과일
...
function updateCleanupTierFilter() {
    ...
    renderTierCheckboxes('cleanupTierCheckboxes', cleanupTiers, '일', 'cleanup', selectedCleanupTiers);
}
...
function renderTierCheckboxes(containerId, tiers, unit, type, selectedSet) {
    ...
    for (let i = tiers.length - 1; i >= 0; i--) {
        if (i === tiers.length - 1) {
            labels.push({ label: `${tiers[i]}${unit} 이상`, min: tiers[i], max: Infinity });
        }
        if (i > 0) {
            labels.push({ label: `${tiers[i - 1]}~${tiers[i]}${unit}`, min: tiers[i - 1], max: tiers[i] });
        }
        if (i === 0) {
            labels.push({ label: `${tiers[0]}${unit} 이하`, min: 0, max: tiers[0] });
        }
    }
        ...
        <div class="monitor-card-new ${marketClass} ${stoppedClass}"
            ...
            <div class="card-body">
                ...
                <div class="expand-info">
                    ...
                    <div class="expand-row cleanup-row ${item.cleanup_status || 'normal'}">📅 마지막등록: ${item.last_cleanup_date || '-'} (${item.days_since_cleanup || 0}일전)</div>
                </div>
            </div>
        </div>
    `;
...
function renderMarketTable() {
    ...
    allData = allData.filter(item => {
        ...
        // 경과일 필터
        if (daysFilterFn && !daysFilterFn(item.days_elapsed)) {
            return false;
        }
        return true;
    });
    ...
    tbody.innerHTML = allData.map((item, idx) => {
        ...
        const daysElapsed = item.days_elapsed;
        let daysElapsedCell = '-';
        let daysColor = '';
...
// 수집 중지
function stopMarketingCollection() {
    stopMarketingSSE();
    stopMarketingPolling();
    marketingTaskId = null;
    document.getElementById('marketingStartBtn').disabled = false;
    document.getElementById('marketingStopBtn').disabled = true;
    showToast('수집 중지됨', 'info');
}
