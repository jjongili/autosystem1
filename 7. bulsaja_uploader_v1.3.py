...
class BulsajaAPIClient(CommonAPIClient):
    ...
    def upload_product(self, product_id: str, group_name: str, market_name: str = "스마트스토어") -> Tuple[bool, str]:
        ...
        # 3. Notices 처리 (쿠팡 강제 재설정 - Meta API 정밀 매칭)
        if market_name == "쿠팡":
            print(f"[INFO] 쿠팡 고시정보 강제 재설정 (Meta API 정밀 매칭)")
            # Meta API '기타 재화' (Index 5) 기준 정확한 명칭 사용
            notices = [
        ...
        if 'attributes' in payload:
             print(f"  - attributes 개수: {len(payload.get('attributes', []))}")
        print(f"  - notices 개수: {len(payload.get('notices', []))}")
        ...
        try:
            ...
            try:
                ...
                pass
            # [디버깅] Payload를 메시지 앞쪽으로 이동 (로그 잘림 방지)
            payload_str = json.dumps(payload, ensure_ascii=False)[:300]
            msg = f"요청:{payload_str}... / HTTP 오류: {e.response.status_code} - 바디:{error_detail}"
            print(f"[ERROR] ❌ HTTP {e.response.status_code} - {error_detail[:100]}")
            return False, msg
        except Exception as e:
            msg = f"예외: {str(e)}"
    ...
    def process_product(self, product: Dict, group_name: str, option_count: int,
        ...
        try:
            ...
            for sku in upload_skus:
                ...
                # 마켓별 할인율 적용 (ESM/11st 3%, 그외 설정값)
                if market_name in ["G마켓/옥션", "11번가"]:
                    d_rate = 3
                else:
                    d_rate = discount_rate # 위에서 random.uniform으로 생성된 값
                ...
                # SKU 객체에 가격 정보 쓰기 (0원 해결 핵심)
                sku['origin_price'] = int(origin_krw) # 원화 원가
            ...
            # [수정] 마켓별 할인율 설정 (ESM/11번가: 3% 고정, 그외: 설정범위 내 랜덤)
            if market_name in ["G마켓/옥션", "11번가"]:
                current_discount_rate = 3
                self.log(f"   ℹ️ {market_name} 할인율 3% 고정 적용")
            else:
                current_discount_rate = int(random.uniform(self.price_settings.discount_rate_min, self.price_settings.discount_rate_max))
            ...
            if min_price_sku:
                ...
                # [중요] 대표상품으로 지정된 옵션은 마켓 정책상 반드시 판매 상태(exclude: False)여야 함
                if min_price_sku.get('exclude') is True:
                    min_price_sku['exclude'] = False
                    self.log(f"   🔓 대표상품 강제 포함 전환 (exclude: True -> False)")
            ...
            if market_name == "스마트스토어":
                ...
                if cat_info:
                    ...
                    self.log(f"   🏷️ SS 카테고리 설정: {cat_info.get('name')}")
                else:
                    self.log(f"   ⚠️ SS 카테고리 검색 실패 (기존 값 유지)")
                ...
                if cat_info:
                    ...
                    self.log(f"   🏷️ ESM 카테고리 강제지정(사용자 요청): {cat_info.get('name')}")
                else:
                ...
                if cat_info:
                    ...
                    self.log(f"   🏷️ ESM 카테고리 수동 지정(최후 폴백): {fixed_full_name}")
                ...
                if cat_info:
                    ...
                    self.log(f"   🏷️ 11번가 카테고리 매칭: {cat_info.get('name')}")

            # [신규] ESM/11번가 추천 옵션 매핑 오류 및 중복 방지 (옵션명 표준화)
            if market_name in ["G마켓/옥션", "11번가"] and 'uploadSkuProps' in detail:
            ...
            if skip_sku_update:
                ...
                if not update_success:
                    ...
                    return result
                self.log(f"   📝 상품 정보(SKU/카테고리) 업데이트 완료")
            ...
            if not upload_success:
                ...
                self.log(f"   ❌ 업로드 실패: {upload_msg[:100]}")
                return result
